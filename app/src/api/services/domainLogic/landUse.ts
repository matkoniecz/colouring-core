...
export async function updateLandUseScat(landUse: LandUseState, landUseUpdate: Partial<LandUseState>): Promise<LandUseState> {
    const landUseGroupUpdate = landUseUpdate.landUseGroup;

    for(let group of landUseGroupUpdate) {
        const isGroupValid = await landUseDataAccess.isLandUseScatGroupAllowed(group);
        if(!isGroupValid) {
    ...
    const landUseOrderUpdate = await landUseDataAccess.getLandUseScatOrderFromGroup(landUseGroupUpdate);
